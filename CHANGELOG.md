...
- **`@batthewz/response-ui-renderer/reference`** — `renderReferenceRegions`, which produces the
...
- **`propLimit`** on `renderReferenceRegions` and `renderViewSpecReference`. The reference caps
...
- Region splicing — how a generated table gets into a hand-written reference — is shared between
  the shipped renderer and the doc generator, and is **not** exported. A host keeping its own
  document with the same markers is a plausible caller and not a demonstrated one; an export is
  far cheaper to add later than to withdraw.
...
- **A component categorised under nothing at all is a throw, not an empty table.**
  `renderReferenceRegions` already threw for a category the caller did not list; it returned an

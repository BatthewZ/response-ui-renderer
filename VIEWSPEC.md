...
| `ProgressBar` | `.Label` `.Value` | `value`: number · `max?`: number · `variant?`: "default"\|"gradient"\|"striped" · `color?`: "accent"\|"success"\|"warning"\|"error" · `size?`: "sm"\|"md"\|"lg" · `animate?`: boolean · `statusLabel?`: string | Takes no children: `.Label` and `.Value` are **siblings** of the bar. Needs `value` and a name. `value` is clamped to 0–`max`; `max` defaults to `100`, and a `max` of `0` or less renders indeterminate. |
| `ProgressRing` | — | `value`: number · `max?`: number · `size?`: number · `thickness?`: number · `color?`: "accent"\|"success"\|"warning"\|"error" | No default accessible name — pass `aria-label`. The centre slot is decorative. `value` is clamped to 0–`max`; `max` defaults to `100`. |
...
| `Table` | `.Head` `.Body` `.Row` `.HeaderCell` `.Cell` | `density?`: "dense"\|"comfortable"\|"spacious" · `striped?`: boolean · `chrome?`: "boxed"\|"rules"\|"plain" · `stickyHeader?`: boolean · `maxHeight?`: number\|string · `tableProps?`: ComponentPropsWithRef<"table"> | Presentational only — it sorts nothing. Rest props land on a wrapper; name it via `tableProps`. |
...
| `VirtualizedDataTable` | — | `data`: T[] · `columns`: ColumnDef<T>[] · `rowHeight`: number · `sort?`: SortState\|null · `defaultSort?`: SortState\|null · `onSortChange?`: (sort: SortState\|null) => void · `sortComparator?`: (a: T, b: T, columnKey: string, direction: "asc"\|"desc") => number · +16 more | As `DataTable`, plus a numeric `rowHeight` and `height`. |
...
10. `dangerouslySetInnerHTML`, `srcDoc`, `ref`, `key` and `__proto__` are dropped. A URL is kept only if its scheme is `http:`, `https:`, `mailto:`, `tel:`, an image `data:` type, or it is relative — so `blob:`, `ftp:`, `sms:` and `data:image/svg+xml` are dropped too. That applies to props spelled like a URL attribute (in any casing), to `viewAllHref`/`to`/`redirect`, and to keys inside a `…Props` bag. `as`, `titleAs` and `headingLevel` pick the element from a fixed allowlist of content elements — not just `script`/`iframe`/`object`/`style` but anything outside it, so prefer a component over an exotic tag. Do not rely on any of them.

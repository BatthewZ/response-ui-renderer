...
| [dev-harness.md](dev-harness.md) | Why the published site's prose pages are documents rather than a docs feature, why a frame owns its chrome and pages draw none, why that chrome must be built from tokens, what a control or a number in the chrome claims by accident, the device-preview trap, and the routes the visual workflow depends on |
